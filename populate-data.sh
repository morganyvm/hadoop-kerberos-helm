...
until kinit -kt /var/keytabs/hdfs.keytab hdfs/nn.bigdata.svc.cluster.local; do sleep 2; done

until (echo > /dev/tcp/nn.bigdata.svc.cluster.local/9000) >/dev/null 2>&1; do sleep 2; done
